...
--53
SELECT contents, url FROM photos LEFT JOIN comments on photos.id = comments.photo_id;

SELECT username, contents FROM comments RIGHT JOIN users on users.id = comments.user_id;

--57
SELECT contents, url FROM photos JOIN comments on photos.id = comments.photo_id WHERE comments.user_id = photos.user_id;

--58
SELECT contents, url, username FROM photos 
JOIN comments on photos.id = comments.photo_id 
JOIN users on users.id = comments.user_id AND users.id = photos.user_id;

--59
select title, name, rating from books 
